...
| AUD-016 | Both schemas implement property-local composite substitution; all three generated runtimes enforce it and emit/read qualified `xsi:type`. JSON Schema structural definitions preserve inheritance through parent `allOf` references and inline concrete discriminator alternatives. | `SchemaAlignmentTests.JsonSchemaUsesAllOfInheritanceAndBoundaryClosure` and `XsdValidatesAssignabilityOrderAndSubtypeBlocking`; runtime negatives; `CrossLanguageChainIntegrationTests`; `Test-GeneratedRuntimes.ps1` carries substituted composites through both language orders and validates every JSON/XML boundary | **Resolved** for the reported schema/runtime mismatch and C# XML write failure. |
| AUD-017 | Schemas are closed and contextual; `AllowSubtypes` now controls property-local exact-versus-assignable behavior for both item references and composites. Abstract declarations warn and are treated as subtype-enabled. JSON Schema keeps every concrete item, required inheritance ancestor, and builtin primitive definition; prunes unreachable model composites; represents model inheritance with `allOf`; and closes final composed values with `unevaluatedProperties:false`. Tagged and property-specific reference restrictions are inline, with every item reference based on the one global closed `Reference`; no helper-suffixed definitions are emitted. Runtimes/validator reject unknown content and preserve external placeholders. | `Cogs2ContractTests.AllowSubtypesAppliesToItemsAndCompositesButNotPrimitives` and `AbstractDeclaredTypesWarnAndAreBuiltAsSubtypeEnabled`; `SchemaAlignmentTests` covers closed inherited instances, property-local discriminator sets, and minimal reachable definition emission; `JsonSchemaDefinitionPruningTests` covers ancestor retention and absence of helper definitions; `XsdValidatesAssignabilityOrderAndSubtypeBlocking`; `InstanceValidatorTests`; generated-runtime tests cover exact and assignable item references | **Resolved.** Schemas and all three generated runtimes enforce the same property-local discriminator set while preserving top-level and external references. The `allOf`/`unevaluatedProperties` schema structure and smaller `$defs` inventory change schema internals only, not the accepted wire contract. |
